...
def log_history(repo: str, action_type: str, status: str, summary: str = ""):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('''
            INSERT INTO history (timestamp, repo, action_type, status, summary)
            VALUES (?, ?, ?, ?, ?)
        ''', (datetime.utcnow().isoformat() + "Z", repo, action_type, status, summary))
        conn.commit()
        conn.close()
...
def get_history():
    ...
    c = conn.cursor()
    c.execute('SELECT * FROM history ORDER BY id DESC LIMIT 100')
    rows = c.fetchall()
    conn.close()
...
def execute_skill(req: SkillExecutionRequest):
    repo = req.params.get('REPO_NAME', '')
    if not repo:
        owner = req.params.get('OWNER', '')
        repo_name = req.params.get('REPO', '')
        repo = f"{owner}/{repo_name}".strip('/')
    if not repo or repo == '/':
        repo = "Unknown Repository"
        
    try:
        res = _execute_skill_internal(req)
        log_history(repo, req.skill_id, "Success", "Action completed successfully")
        return res
    except Exception as e:
        log_history(repo, req.skill_id, "Failed", str(e))
        raise e

def _execute_skill_internal(req: SkillExecutionRequest):
...
def execute_raw_query(req: QueryRequest):
    owner, repo_name = extract_repo_from_query(req.query)
    repo = f"{owner}/{repo_name}".strip('/')
    if not repo or repo == '/':
        repo = "Raw SQL Query"
    try:
        res = _execute_raw_query_internal(req)
        log_history(repo, "raw_query", "Success", "Query executed successfully")
        return res
    except Exception as e:
        log_history(repo, "raw_query", "Failed", str(e))
        raise e

def _execute_raw_query_internal(req: QueryRequest):
    update_global_tokens(req)

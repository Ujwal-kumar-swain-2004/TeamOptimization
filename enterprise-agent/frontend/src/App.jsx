...
function App() {
  ...
  const [cachedQueries, setCachedQueries] = useState([
    "SELECT * FROM github.issues WHERE state = 'open' LIMIT 50",
    "SELECT title, user FROM github.pulls WHERE repo = 'tambo' AND state = 'closed'",
    "SELECT count(*) FROM sentry.issues WHERE project = 'frontend'",
    "SELECT id, message FROM discord.messages WHERE channel = 'incidents' ORDER BY timestamp DESC",
    "SELECT issue_key, status FROM jira.tickets WHERE assignee = 'me'",
    "SELECT * FROM github.commits WHERE author = 'arya' LIMIT 10",
    "SELECT title, url FROM github.issues WHERE labels CONTAINS 'bug'",
    "SELECT count(id) FROM sentry.events WHERE level = 'error' AND environment = 'production'",
    "SELECT user, action FROM github.audit_log WHERE action = 'repo.create'",
    "SELECT id, name FROM discord.channels WHERE guild_id = '123456789'",
    "SELECT key, summary FROM jira.issues WHERE project = 'TOP' AND status = 'In Progress'",
    "SELECT * FROM github.releases ORDER BY published_at DESC LIMIT 5",
    "SELECT title, state, created_at FROM github.pulls WHERE author = 'bot'",
    "SELECT event_id, message, culprit FROM sentry.events WHERE release = 'v1.0.2'",
    "SELECT username, roles FROM discord.members WHERE role = 'admin'",
    "SELECT issue_id, comments FROM jira.comments WHERE author = 'john.doe'",
    "SELECT * FROM github.workflows WHERE status = 'failed' LIMIT 20",
    "SELECT login, contributions FROM github.contributors WHERE repo = 'enterprise-agent'",
    "SELECT issue_key, resolution FROM jira.tickets WHERE status = 'Done' AND resolved >= '2026-05-01'",
    "SELECT id, project, first_seen FROM sentry.issues WHERE is_unhandled = true",
    "SELECT author, message, timestamp FROM discord.messages WHERE content LIKE '%down%'",
    "SELECT html_url, body FROM github.issues WHERE comments > 10",
    "SELECT count(*) FROM github.pulls WHERE merged = true AND merged_at >= '2026-05-20'",
    "SELECT key, priority, assignee FROM jira.issues WHERE priority = 'Highest' AND status != 'Closed'"
  ]);
        ...
        <aside className="sidebar">
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div
              ...
              data-tooltip="View the history of all actions performed."
            >
              <History size={18} />
              <span>History</span>
            </div>
      ...
      <main className="main-content" style={{ padding: isLanding ? '0' : '40px' }}>
        ...
        {view === 'landing' ? (
              ...
              <div className="nav-container">
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1px', lineHeight: '1' }}>
                    ...
                    <img src="/dashboard/logo_tree.png" alt="O" style={{ width: '36px', height: '36px', display: 'block', flexShrink: 0 }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
            <header>
              <h1 style={{ fontSize: '24px', fontWeight: '800', background: 'linear-gradient(120deg, var(--accent), #818cf8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <History size={24} style={{ color: 'var(--accent)' }} /> Action History
              </h1>
              <p style={{ color: 'var(--text-dim)', fontSize: '14px', marginTop: '4px' }}>
                A chronological log of all diagnostic actions and SQL queries performed across your repositories.
              </p>
            </header>
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {historyLoading ? (
                ...
                <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-dim)' }}>
                  ...
                  <p>No history found yet. Run an action on the Dashboard to see it here.</p>
                </div>
              ) : (
                  ...
                  <table style={{ minWidth: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                    <thead>
                      <tr style={{ background: 'var(--bg-input)' }}>
                        <th style={{ padding: '10px 16px', color: 'var(--text-main)', fontWeight: '700', textAlign: 'left' }}>Timestamp</th>
                        <th style={{ padding: '10px 16px', color: 'var(--text-main)', fontWeight: '700', textAlign: 'left' }}>Repository</th>
                        <th style={{ padding: '10px 16px', color: 'var(--text-main)', fontWeight: '700', textAlign: 'left' }}>Action</th>
                        <th style={{ padding: '10px 16px', color: 'var(--text-main)', fontWeight: '700', textAlign: 'left' }}>Status</th>
                        <th style={{ padding: '10px 16px', color: 'var(--text-main)', fontWeight: '700', textAlign: 'left' }}>Summary</th>
                      </tr>
                    </thead>
                    <tbody>
                      {historyLogs.map((log) => {
                        ...
                        return (
                          <tr key={log.id} style={{ borderBottom: '1px solid var(--border)' }}>
                            ...
                            <td style={{ padding: '10px 16px', color: 'var(--text-main)', fontWeight: '600' }}>
                              {log.repo}
                            </td>
                            <td style={{ padding: '10px 16px', color: 'var(--accent)' }}>
                              {log.action_type === 'raw_query' ? 'Custom SQL' : log.action_type.replace(/_/g, ' ')}
                            </td>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: '24px' }}>
              ...
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '16px', justifyContent: 'space-between' }}>
                ...
                <div style={{ flex: 1, minHeight: '150px', maxHeight: '250px', overflowY: 'auto', background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: '6px', padding: '8px' }}>
                  {cachedQueries.length > 0 ? (
                    <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      {cachedQueries.map((q, i) => (
                        <li key={i} style={{ fontSize: '11px', fontFamily: 'monospace', color: 'var(--text-dim)', padding: '6px', background: 'var(--bg-card)', borderRadius: '4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={q}>
                          <span style={{ color: 'var(--accent)', marginRight: '6px' }}>[{i + 1}]</span> {q}
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--text-dim)', fontSize: '12px' }}>
                      No queries cached.
                    </div>
                  )}
                </div>
